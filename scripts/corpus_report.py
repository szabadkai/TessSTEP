...
def aggregate(cases, baseline_present=False):
    ...
    return {"sources": dict(sorted(sources.items())), "stages": stages,
...
def render(report):
    ...
    for case in cases:
        ...
        detail = {k: case[k] for k in ("expectations", "expectation_failures", "schemas", "entity_counts", "missing_references", "diagnostics", "diagnostic_counts", "stderr", "schema_result", "schema_stderr", "product_result", "product_stderr", "determinism", "baseline_change", "output_sha256", "sha256", "bytes") if k in case}
        name = case["paths"][0]
...
<p class="note">Physical acceptance is not CAD conformance. Rejected adversarial files may be correct. <b>Passed</b> means reviewed expectations matched; <b>compatible</b> means no regression against an observation baseline; <b>unreviewed</b> has no oracle.<br>Schema/product checks require configured validators. Geometry and tessellation have constructed-model tests but no external STEP integration in this runner. <b>Not configured, not integrated and not run are not passes.</b></p>
<p><a href="latest.json">JSON</a> · <a href="summary.md">Markdown summary</a> · <a href="junit.xml">JUnit</a> · <a href="cases.csv">CSV</a></p>
<h2>Coverage by stage</h2><div class="panel"><table><tr><th>Stage</th><th>Executed / unique inputs</th><th>Outcomes</th></tr>{stage_rows}</table></div>
