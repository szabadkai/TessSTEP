...
def signature(case):
    return {key: case.get(key) for key in ("status", "stages", "diagnostic_counts", "entity_count", "missing_references")}
...
def render(report):
    ...
    for case in report["cases"]:
        ...
        rows.append(f'<tr data-status="{esc(case["status"])}"><td>{paths}</td><td>{esc(case["status"])}</td>'
                    + "".join(f'<td>{esc(case["stages"][s])}</td>' for s in STAGES[:2])
...
<p><b>Schema, geometry and tessellation: not implemented.</b> Rejected adversarial files may be correct behavior. These observations are not conformance scores. External resources and signatures are not verified.</p>
...
<table id="files"><thead><tr><th>File paths</th><th>Observed status</th><th>Physical parse</th><th>References</th><th>Entities</th><th>Seconds</th><th>Diagnostics (first 20)</th></tr></thead><tbody>{''.join(rows)}</tbody></table>
...
def main():
    ...
    for path in sorted([ROOT / "Cargo.toml", ROOT / "Cargo.lock", Path(__file__), *ROOT.glob("crates/**/*.rs"), *ROOT.glob("crates/**/Cargo.toml"), *ROOT.glob("tools/**/*.rs"), *ROOT.glob("tools/**/Cargo.toml")]):
    ...
    with tempfile.TemporaryDirectory(prefix="tessstep-corpus-") as directory:
        ...
        for index, case in enumerate(cases, 1):
            case.update(inspect(snapshot, root / case["paths"][0], args.timeout))
    ...
    report = {"format_version": 1, "run_id": run_id, "corpus_root": str(root), "source_fingerprint": source.hexdigest(),
              "binary_sha256": binary_hash, "timeout_seconds": args.timeout, "summary": summary, "cases": cases,
